...
class KeyManager:
    ...
    def _score_keys(self, keys: list[APIKey]) -> list[tuple[APIKey, float]]:
        ...
        # last_used: None means never used — we treat that as most desirable (0 penalty)
        recency = [
            (now - k.last_used).total_seconds() if k.last_used else float("inf")
            for k in keys
        ]
    ...
    async def delete_key(self, key_id: int) -> bool:
        """
        Soft-delete by disabling. We keep the row for audit history.
        If you need hard deletes, add a `deleted_at` timestamp column.
        """
        key = await self._get_by_id(key_id)
        if not key:
            return False
    ...
    async def list_keys(self, skip: int = 0, limit: int = 50) -> tuple[list[APIKey], int]:
        """Return paginated list of all keys and total count."""
        count_result = await self.db.execute(select(func.count(APIKey.id)))
        total = count_result.scalar_one()

        result = await self.db.execute(
            select(APIKey).order_by(APIKey.id).offset(skip).limit(limit)
        )
    ...
    async def get_fleet_stats(self) -> dict:
        ...
        result = await self.db.execute(
            select(
                ...
                func.sum(
                    (APIKey.status == KeyStatus.ACTIVE).cast(int)
                ).label("active"),
                func.sum(
                    (APIKey.status == KeyStatus.RATE_LIMITED).cast(int)
                ).label("rate_limited"),
                func.sum(
                    (APIKey.status == KeyStatus.DISABLED).cast(int)
                ).label("disabled"),
                func.sum(APIKey.total_requests).label("total_requests"),
                func.sum(APIKey.total_failures).label("total_failures"),
